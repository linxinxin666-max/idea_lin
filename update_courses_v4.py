...
def generate_course_content(title):
    ...
    search_keyword = title[:15] if len(title) > 15 else title
    search_url = f"https://lifexue.com/course?enter_method=tab&search={quote(search_keyword)}"
    
    professional = random.choice(professional_templates).format(title, search_keyword, search_url)
    friendly = random.choice(friendly_templates).format(title, search_keyword, search_url)
