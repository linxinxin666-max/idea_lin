...
def generate_courses():
    ...
    while len(all_courses) < 474:
        ...
        if theme:
            search_keywords.append(theme)
        if "套餐" in title:
            search_keywords.append("套餐")
        if "运营" in title:
            search_keywords.append("运营")
        
        search_query = " ".join(search_keywords[:2])  # 用前两个关键词搜索
        encoded_query = quote(search_query)
        
        # 尝试几种搜索链接格式，或者回到课程列表页
        course_url = f"{BASE_URL}/course?enter_method=tab&search={encoded_query}"
