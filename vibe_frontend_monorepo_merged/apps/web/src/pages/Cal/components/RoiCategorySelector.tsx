...
import type { RoiState } from '../types'
...
interface RoiCategorySelectorProps {
  state: RoiState
  onChange: (updates: Partial<RoiState>) => void
  industryClassName?: string
  industrySpan?: boolean
}

export function RoiCategorySelector({
  state,
  onChange,
  industryClassName,
  industrySpan = true,
}: RoiCategorySelectorProps) {
  ...
  useEffect(() => {
    ...
    const changed = Object.entries(updates).some(([key, value]) => state[key as keyof RoiState] !== value)
    if (changed) onChange(updates)
  }, [hierarchy, state.industry, state.categoryL1, state.categoryL2, state.categoryL3, state.commissionRule, state.rateGroup, state.commissionRate, state.commissionRateEdited])
      ...
      <div className="form-row form-row-span">
        ...
        <div className="select-group">
          <div className="select-item">
            ...
            <select value={selectedL1} onChange={(e) => onChange({ categoryL1: e.target.value, categoryL2: '', categoryL3: '' })}>
              {l1Keys.map((k) => (
          ...
          <div className="select-item">
            ...
            <select
              ...
              onChange={(e) => onChange({ categoryL2: e.target.value, categoryL3: '' })}
            >
              {l2Keys.map((k) => (
                <option key={k} value={k}>
                  {k}
          ...
          <div className="select-item">
            ...
            <select value={(l3Options.some((o) => o.name === state.categoryL3) ? state.categoryL3 : l3Options[0]?.name) || ''} onChange={(e) => onChange({ categoryL3: e.target.value })}>
              {l3Options.map((opt) => (
...
function PoiSelector({ state, onChange }: { state: RoiState; onChange: (updates: Partial<RoiState>) => void }) {
  const data = getCommissionData()
  const poi = data.poi
  if (!poi) return null
  ...
  const handlePoiChange = (field: 'poiL1' | 'poiL2' | 'poiL3', value: string) => {
    const updates: Partial<RoiState> = { [field]: value }
