...
import type { ScanIntentState } from '../types'
...
interface ScanIntentCategorySelectorProps {
  state: ScanIntentState
  onChange: (updates: Partial<ScanIntentState>) => void
...
export function ScanIntentCategorySelector({
  state,
  onChange,
  industryClassName,
  industrySpan = true,
...
}: ScanIntentCategorySelectorProps) {
  ...
  }, [hierarchy, activeIndustry, state.industry, state.categoryL1, onChange])
  ...
  useEffect(() => {
    ...
    const updates: Partial<ScanIntentState> = {}
    ...
    if (selectedOpt) {
      ...
      if (selectedOpt.rule === 'poi') {
        ...
        if (!state.commissionRateEdited) {
          updates.commissionRate = rateGroup
        }
      }
    }

    const changed = Object.entries(updates).some(([key, value]) => state[key as keyof ScanIntentState] !== value)
    if (changed) onChange(updates)
  }, [hierarchy, state.industry, state.categoryL1, state.categoryL2, state.categoryL3, state.commissionRateEdited, onChange])
    ...
    <>
      ...
      {showCategory && (
        <div className="form-row form-row-span">
          {showCategoryLabel && <label>类目</label>}
          <div className="select-group">
            <div className="select-item">
              <div className="select-label">一级类目</div>
              <select value={selectedL1} onChange={(e) => onChange({ categoryL1: e.target.value, categoryL2: '', categoryL3: '', commissionRateEdited: false })}>
                {l1Keys.map((k) => (
...
function PoiSelector({ state, onChange }: { state: ScanIntentState; onChange: (updates: Partial<ScanIntentState>) => void }) {
  const data = getCommissionData()
  const poi = data.poi
  if (!poi) return null
  ...
  const handlePoiChange = (field: 'poiL1' | 'poiL2' | 'poiL3', value: string) => {
    const updates: Partial<ScanIntentState> = { [field]: value }
